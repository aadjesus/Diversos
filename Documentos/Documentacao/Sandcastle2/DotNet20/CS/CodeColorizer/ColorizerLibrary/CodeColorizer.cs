...
//
//      Added missing support for the <code lang="xxx"> tag.
//
...
//          Reworked the keyword lists to share common keywords amongst
//          similar languages.
//
//          Added support for C#, VB.NET, and J#.  Also added "name" attribute
//          to each "language" element to specify a friendly name.
//
//          Removed the preprocessor keyword list and added a generic handler
//          for preprocessor directives in each language that needed it.
//
//          Added <include.file> handler for C and C++.
//
//          Added number handler to each languages and the XSL transformation.
//
//          Reworked the VB comment detection to prevent it double-spacing
//          the comments and moved REM keyword into it as well.
// 
...
namespace ColorizerLibrary
{
    ...
    public class CodeColorizer
    {
        ...
        private class CollapsibleRegion
        {
		...
		// Syntax description file and friendly name dictionary
		private XmlDocument languageSyntax;
        private Dictionary<string, string> friendlyNames;
        ...
        /// </summary>
        /// <value>The default is "CopyCode.gif".</value>
                    ...
                    if(outlining)
                    {
                        ...
                        if(m.Success)
                        {
                            ...
                            if(m.Groups[1].Value.ToLower(
                              CultureInfo.InvariantCulture).StartsWith("#else"))
                            {
		...
		private static void BuildKeywordRegExp(XmlNode languageNode)
		{
            XmlNodeList keywords;
			XmlNode preNode, postNode;
            string[] words = new string[100], expression = new string[7];
            int idx;

            // Set the common regular expression parts
            expression[0] = @"\b";      // Start word boundary
            expression[2] = "(";        // Opening
            expression[4] = ")";        // Closing
            expression[6] = @"\b";      // End word boundary

            // Iterate keyword lists
			foreach(XmlNode keywordListNode in languageNode.SelectNodes(
          ...
          XmlNode contextNode)
		{
            ...
            XmlNode rootNode, familyNode, kwList, node;
			...
			foreach(XmlNode ruleNode in contextNode.ChildNodes)
			{
				...
				else if(ruleNode.Name == "keyword")
				{
					...
					kwList = rootNode.SelectSingleNode(
                        "keywordlists/keywordlist[@id=\"" +
                        familyNode.Value + "\"]");
		...
		private void BuildRules(XmlNode languageNode)
		{
			...
			CodeColorizer.BuildKeywordRegExp(languageNode);
		...
		private void BuildSyntax()
		{
			...
			if(needBuildNode == null || needBuildNode.Value == "yes")
			{
                ...
                foreach(XmlNode languageNode in highlightNode.SelectNodes(
                  "languages/language"))
                {
          ...
          string matchText)
		{
			...
			foreach(XmlNode ruleNode in contextNode.ChildNodes)
			{
				...
				{
					node = rootNode.SelectSingleNode(
                        "keywordlists/keywordlist[@id=\""
		...
		private String ReplaceByCode(Match match)
		{
            ...
            bool preserveNumbering = numberLines,
                preserveOutlining = outlining;
            string title = null;
            ...
            try
            {
                ...
                // Find the language in the language file.  If not found,
                // we'll just format it neatly and optionally number it and
                // add collapsible regions.
			    languageNode = languageSyntax.SelectSingleNode(
                    "/highlight/languages/language[@id=\"" + language + "\"]");
                ...
                else
                {
                    ...
                    // specified.
                    if(languageNode.Attributes["tabSize"] != null)
                    ...
                    inBox = (String.Compare(tag, "pre", true,
                ...
                if(inBox)
                {
                    ...
                    {
                        if(useDefaultTitle)
                            friendlyNames.TryGetValue(language, out title);
