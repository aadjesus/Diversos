...
{
    /// <summary>
    /// This class was taken from Paul Stovells codeproject
    /// article, and should be used to wrap several bound 
    /// controls within a XAML Window. 
    /// The Validate() method may be called to see the invalid objects
    /// See http://www.codeproject.com/KB/WPF/wpfErrorProvider.aspx 
    /// for the full details
    /// </summary>
    public class ErrorProvider : Decorator
    {
        #region Data
        private delegate void FoundBindingCallbackDelegate(
            FrameworkElement element, Binding binding, DependencyProperty dp);
        private FrameworkElement _firstInvalidElement;
        private Dictionary<DependencyObject, Style> 
            _backupStyles = new Dictionary<DependencyObject, Style>();
        #endregion
        ...
        {
            this.DataContextChanged += new 
                DependencyPropertyChangedEventHandler(
                ErrorProvider_DataContextChanged);
            this.Loaded += new 
                RoutedEventHandler(ErrorProvider_Loaded);
        }
        #endregion
        ...
        /// <summary>
        /// Returns the first element that this error provider has 
        /// labelled as invalid. This method is useful to set the 
        /// users focus on the first visible error field on a page.
        /// </summary>
        /// <returns></returns>
        ...
        /// We have a call to Validate here that way errors appear from the very 
        ...
        /// </summary>
        private void ErrorProvider_DataContextChanged(object sender,
            DependencyPropertyChangedEventArgs e)
        {
            if (e.OldValue != null && e.OldValue is INotifyPropertyChanged)
            {
                ((INotifyPropertyChanged)e.NewValue).PropertyChanged
                    -= new PropertyChangedEventHandler(DataContext_PropertyChanged);
            }
        ...
        /// <summary>
        /// Clears any error messages and returns a list of all bindings 
        /// on the current form/page. This is simply so it can be reused 
        ...
        private List<Binding> ClearInternal()
        {
            ...
            FindBindingsRecursively(
                    this.Parent,
                    delegate(FrameworkElement element, 
                    Binding binding, DependencyProperty dp)
                    {
                        // Remember this bound element. We'll use this to 
        ...
        /// <summary>
        /// Called when the PropertyChanged event is raised from the 
        /// object we are bound to - that is, our data context.
        /// </summary>
        ...
        /// <summary>
        /// Recursively goes through the control tree, looking for 
        /// bindings on the current data context.
        /// </summary>
        ...
        private void FindBindingsRecursively(DependencyObject element, 
            FoundBindingCallbackDelegate callbackDelegate)
        {

            // See if we should display the errors on this element
            MemberInfo[] members = 
                element.GetType().GetMembers(BindingFlags.Static |
                    BindingFlags.Public |
            ...
            foreach (MemberInfo member in members)
            {
                ...
                // Check to see if the field or property we were 
                //given is a dependency property
                ...
                if (dp != null)
                {
                    ...
                    if (bb != null)
                    {
                        // This element has a DependencyProperty that we know of that 
                        //is bound to the property we're interested in. 
            ...
            if (element is FrameworkElement || element is FrameworkContentElement)
            {
                foreach (object childElement in LogicalTreeHelper.GetChildren(element))
                {
                    ...
                    {
                        FindBindingsRecursively((DependencyObject)childElement, 
                            callbackDelegate);
                    }
                }
            }
