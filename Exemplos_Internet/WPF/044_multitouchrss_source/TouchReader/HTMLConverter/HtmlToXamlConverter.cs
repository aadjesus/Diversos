...
namespace Ricciolo.PaperBoy.UI.HTMLConverter
{
    using System;
    using System.Xml;
    ...
    public static class HtmlToXamlConverter
    {
        ...
        {
            XmlElement xamlFlowDocumentElement = ConvertHtmlToXamlElement(htmlString, asFlowDocument);
        ...
        /// Well-formed xml representing XAML equivalent for the input html string.
        /// </returns>
        public static XmlElement ConvertHtmlToXamlElement(string htmlString, bool asFlowDocument)
        {
        ...
        /// Well-formed xml representing XAML equivalent for the input html string.
        /// </returns>
        public static XmlElement ConvertHtmlToXamlElement(XmlDocument html, bool asFlowDocument)
        {
            XmlElement htmlElement = html.DocumentElement;
            // Decide what name to use as a root
            string rootElementName = asFlowDocument ? HtmlToXamlConverter.Xaml_FlowDocument : HtmlToXamlConverter.Xaml_Section;
        ...
        private static XmlNode AddBlock(XmlElement xamlParentElement, XmlNode htmlNode, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            else if (htmlNode is XmlElement)
            {
                ...
                switch (htmlElementName)
                {
                    ...
                    case "img":
                        AddImage(xamlParentElement, htmlElement, inheritedProperties, stylesheet, sourceContext, false);
                        break;
        ...
        private static void AddImage(XmlElement xamlParentElement, XmlElement htmlElement, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext, bool inline)
        {
            XmlElement xamlUIElement = xamlParentElement.OwnerDocument.CreateElement(null, (inline) ? Xaml_InlineUIContainer : Xaml_BlockUIContainer, _xamlNamespace);
            xamlParentElement.AppendChild(xamlUIElement);

            // Create Image into UIContainer
            XmlElement xamlImageElement = xamlParentElement.OwnerDocument.CreateElement(null, typeof(Image).Name, _xamlNamespace);
            xamlUIElement.AppendChild(xamlImageElement);

            // Add src attribute
            string src = htmlElement.GetAttribute("src");
            if (!String.IsNullOrEmpty(src))
            {
                XmlElement imageSourceElement = xamlParentElement.OwnerDocument.CreateElement(null, "Image.Source", _xamlNamespace);
                xamlImageElement.AppendChild(imageSourceElement);
                XmlElement bitmapImageElement = xamlParentElement.OwnerDocument.CreateElement(null, typeof(System.Windows.Media.Imaging.BitmapImage).Name, _xamlNamespace);
                imageSourceElement.AppendChild(bitmapImageElement);
                bitmapImageElement.SetAttribute("UriSource", src);
                //bitmapImageElement.SetAttribute("CreateOptions", System.Windows.Media.Imaging.BitmapCreateOptions.DelayCreation.ToString());
                bitmapImageElement.SetAttribute("CacheOption", System.Windows.Media.Imaging.BitmapCacheOption.OnDemand.ToString());
            }

            bool hasW = false, hasH = false;
            // Add size
            string w = htmlElement.GetAttribute("width");
            double width;
            if (TryGetLengthValue(w, out width))
            {
                hasW = true;
                xamlImageElement.SetAttribute(Xaml_Width, width.ToString());
            }

            string h = htmlElement.GetAttribute("height");
            double height;
            if (TryGetLengthValue(h, out height))
            {
                hasH = true;
                xamlImageElement.SetAttribute("Height", height.ToString());
            }
    
            if (hasW && hasH)
                xamlImageElement.SetAttribute("Stretch", Stretch.Fill.ToString());
            else
                xamlImageElement.SetAttribute("Stretch", Stretch.None.ToString());
        }
        ...
        private static Hashtable GetElementProperties(XmlElement htmlElement, Hashtable inheritedProperties, out Hashtable localProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            switch (elementName)
            {
                ...
                case "font":
                    ...
                    if (attributeValue != null)
                    {
                        double fontSize = Math.Round(double.Parse(attributeValue) * (16.0 / 3.0), 2);
                        if (fontSize < 1.0)
                        {
                            fontSize = 1.0;
                        }
