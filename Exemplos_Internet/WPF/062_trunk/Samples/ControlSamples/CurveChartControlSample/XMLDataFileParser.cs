...
namespace CurveChartControlSample
{
	...
	static class XMLDataFileParser
	{
				...
				while (reader.Read())
				{
					if (reader.NodeType == XmlNodeType.Element)
					{
						...
						if (reader.Name == "SampledCurve")
							item = ParseSampledCurve(reader, out xAxisLabel);
						else if (reader.Name == "ScatteredPoints")
							item = ParseScatteredPoints(reader, out xAxisLabel);
		...
		/// <param name="xAxisLabel">The x axis label.</param>
		/// <returns></returns>
		static ItemData ParseSampledCurve(XmlTextReader reader, out string xAxisLabel)
		{
		...
		/// <param name="xAxisLabel">The x axis label.</param>
		/// <returns></returns>
		static ItemData ParseScatteredPoints(XmlTextReader reader, out string xAxisLabel)
		{
			Debug.Assert(reader.NodeType == XmlNodeType.Element, "reader.NodeType == XmlNodeType.Element");
