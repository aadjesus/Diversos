...
void CLabel::OnPaint() 
{
	...
	HBITMAP l_hbmpBitmap;
	l_hbmpBitmap = GetBitmap();
	...
	CBitmap*	l_pOldMemoryBitmap = NULL;
	
	if (!m_bTransparent)
	{
		//pDCMem = new CDC;
		helperpDCMem.CreateCompatibleDC(&dc);
		pDCMem = &helperpDCMem;
	...
	//code to manage pic with transparent background... mod by Arf...
	if (m_bTransparentPic)
	{
		#define ROP4_TRANSPARENTBLIT		0xCCAA0020
		#define SRCMASK 					0x00220326

		pDCMem->FillSolidRect( &rc, m_bkcol ) ;
		CDC 	 l_MemoryDC ;
		CDC 	 l_MaskDC ;
		CBitmap  l_MaskBitmap ;
		l_MaskBitmap.CreateBitmap( rc.Width(), rc.Height(), 1, 1, NULL ) ;
		l_MaskDC.CreateCompatibleDC( &dc ) ;
		l_MemoryDC.CreateCompatibleDC( &dc ) ;
		l_pOldMemoryBitmap = l_MemoryDC.SelectObject( CBitmap::FromHandle( l_hbmpBitmap ) ) ;
		pDCMem->BitBlt( 0, 0, rc.Width(), rc.Height(), &l_MemoryDC, 0, 0, SRCCOPY ) ;
		l_MemoryDC.SelectObject( l_pOldMemoryBitmap ) ;
		
		CBitmap* l_pOldMaskBitmap = l_MaskDC.SelectObject( &l_MaskBitmap ) ;
		COLORREF l_crOldBack = pDCMem->SetBkColor( m_bkcol ) ;

		l_MaskDC.BitBlt( 0, 0, rc.Width(), rc.Height(), pDCMem, 0, 0, SRCCOPY ) ;

		pDCMem->SetBkColor( l_crOldBack ) ;
		...
		if( ! dc.MaskBlt( 0, 
			...
			ROP4_TRANSPARENTBLIT ) )
			{
				...
				l_CopyBitmap.CreateCompatibleBitmap( &dc, rc.Width(), rc.Height() ) ;
				CBitmap* l_pOldCopyBitmap = l_CopyDC.SelectObject( &l_CopyBitmap ) ;

				l_CopyDC.BitBlt(   0, 0, rc.Width(), rc.Height(), &dc,  0, 0, SRCCOPY  ) ;
				l_CopyDC.BitBlt(   0, 0, rc.Width(), rc.Height(), &l_MaskDC,   0, 0, SRCAND   ) ;
				pDCMem->BitBlt( 0, 0, rc.Width(), rc.Height(), &l_MaskDC,   0, 0, SRCMASK  ) ;
				l_CopyDC.BitBlt(   0, 0, rc.Width(), rc.Height(), pDCMem, 0, 0, SRCPAINT ) ;
				dc.BitBlt(  0, 0, rc.Width(), rc.Height(), &l_CopyDC,   0, 0, SRCCOPY  ) ;
	...
	// Fill in background if not transparent
	if (!m_bTransparent && !m_bTransparentPic)
	{
	...
	// Draw the line if set to true... (mod by Arf)

	if (m_bBevelline)
	{
		CSize txtSize;
		txtSize=pDCMem->GetTextExtent(strText);
		txtSize.cx+=7; // I like some space 
		switch (dwFlags)
		{
			case (DT_LEFT | DT_WORDBREAK): 
				pDCMem->Draw3dRect (
				...
				break;
			case (SS_CENTER | DT_WORDBREAK):
				pDCMem->Draw3dRect (
					0, rc.Height()/2,      
					(rc.Width()-txtSize.cx)/2, 2,         // Left Line...
					::GetSysColor(COLOR_3DSHADOW),
					::GetSysColor(COLOR_3DHIGHLIGHT) );
				pDCMem->Draw3dRect (
					(rc.Width() + txtSize.cx)/2, rc.Height()/2,     
					(rc.Width()-txtSize.cx)/2, 2,         //  Right Line :)
					::GetSysColor(COLOR_3DSHADOW),
					::GetSysColor(COLOR_3DHIGHLIGHT) );
				break;
			case (DT_RIGHT | DT_WORDBREAK):
				pDCMem->Draw3dRect (
					0, rc.Height()/2,     
					rc.Width()-txtSize.cx, 2,        
					::GetSysColor(COLOR_3DSHADOW),
	...
	if((GetFocus() == this) && (IsBuddy()))
	{
		DrawFocusRect(pDCMem->m_hDC,rc);
	}
	if (!m_bTransparent)
	{
		if (!m_bTransparentPic)
...
CLabel& CLabel::SetBevelLine(BOOL bBevelline)
{	

	m_bBevelline = bBevelline;
	UpdateSurface();
	return *this;
}
...
void CLabel::SetTransparentBk(COLORREF Col, BOOL bTransparentPic)
{
	m_bkcol=Col;
	m_bTransparentPic = bTransparentPic;
	m_bTransparent = FALSE; //these aren't compatible!
}
void CLabel::OnSetFocus(CWnd* pOldWnd)
{
	CStatic::OnSetFocus(pOldWnd);
	InvalidateRect(NULL);
}

void CLabel::OnKillFocus(CWnd* pNewWnd)
{
...
UINT CLabel::OnGetDlgCode()
{
	return DLGC_WANTALLKEYS;
	UINT uRet=CStatic::OnGetDlgCode();

	if (LPMSG pMsg = (LPMSG)GetCurrentMessage()->lParam){
		if ((pMsg->message == WM_KEYDOWN || pMsg->message==WM_CHAR) && (pMsg->wParam == VK_RETURN))
...
void CLabel::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch(nChar)
	{ /* nChar */
	case VK_TAB:
		...
		if( GetKeyState(VK_SHIFT) >= 0)
			GetParent()->PostMessage( WM_NEXTDLGCTL, 0, false );
		else
			GetParent()->PostMessage( WM_NEXTDLGCTL, 1, false );
		break;
	...
	case VK_SPACE:
		if(IsBuddy())
			GetBuddy()->SetCheck(!GetBuddy()->GetCheck());
		break;
	} /* nChar */
...
CButton* CLabel::GetBuddy(void)
{
	return m_pBuddyButton;
}

BOOL CLabel::IsBuddy(void)
{
	if (m_pBuddyButton != NULL)
		return TRUE;
	else
		return FALSE;
}

void CLabel::SetBuddy(CButton* pByddyButton)
{
	m_pBuddyButton = pByddyButton;
}
