...
// Mod by Arf
//
// Nov. 03, 2005 - Merged with button modification
// Oct 2004 -> 
//	July 2005	 - Added some new features (by Arf): new coloums styles (hidden,fixed ecc)
//				   new editable styles: editbox,combobox,button...
// 
//
// Oct. 20, 2005 - Fixed iusse with insercoloumns
//
// Nov. 03, 2005 - Merged with button modification
///////////////////////////////////////////////////////////////////////////////
...
CReportCtrl::CReportCtrl(): m_pWndEdit(NULL)
{
	...
	m_pWndButton = new CButton;
	VERIFY(m_pWndButton != NULL);
	m_pszSeparator = NULL;
	m_bAllowEdit = FALSE;
	m_ptEditting.x = -1;
	m_ptEditting.y = -1;
...
BEGIN_MESSAGE_MAP(CReportCtrl, CListCtrl)
	...
	//}}AFX_MSG_MAP
	ON_WM_KILLFOCUS()
	ON_EN_KILLFOCUS(15550, OnKillFocusChild)
	ON_BN_CLICKED(ID_BUTTON, OnBnClickedButton)
END_MESSAGE_MAP()
...
BOOL CReportCtrl::SetColumnHeader(const CString& strHeadings)
{
	...
	for (int i = 0; i < aLong.GetSize(); i++)
	{
		...
		if (aShort.GetSize() > 0)
		{
			...
			BOOL bLock  = aShort.GetSize() > 3 ? _ttoi(aShort[3]) : 0;
			BOOL bEditable = aShort.GetSize() > 4 ? _ttoi(aShort[4]) : 1;
			int EditStyle = aShort.GetSize() > 5 ? _ttoi(aShort[5]) : 0; 

			m_bLockColumn.push_back(bLock);
			m_bEditableColumn.push_back(bEditable);
			m_ColumnEditStyle.push_back(EditStyle);
...
{
	EndEdit(TRUE);
	if (m_bLockColumn.size() > nCol)
	{
		vector <BOOL>::iterator inx;
		inx = m_bLockColumn.begin();
		m_bLockColumn.erase(inx+nCol);
	}
...
BOOL CReportCtrl::StartEdit(int nItem, int nSubItem)
{
	...
	if (m_bEditableColumn.size() > nSubItem)
		if (!m_bEditableColumn[nSubItem])
			return TRUE;
	EndEdit(TRUE);
	...
	const DWORD	EFMT = _GetHeaderTextEditFormat(nSubItem);
	switch(EFMT) 
	{
	case RC_EDIT_CTRL:
		{
	...
	case RC_COMBO_CTRL:
		{
			int pos = 0;
			CString strTxt = GetItemText(m_ptEditting.x, m_ptEditting.y);
			
			if (m_pWndCombo->GetSafeHwnd() != NULL)
				m_pWndCombo->DestroyWindow();

			if (!m_pWndCombo->Create(WS_CHILD|CBS_DROPDOWNLIST|WS_VSCROLL|CBS_AUTOHSCROLL | WS_VISIBLE /*| WS_VSCROLL*/ | WS_BORDER |CBS_HASSTRINGS   , CRect(0, 0, 1, 1), this, 15550))
				return FALSE;
			...
			rcEdit.DeflateRect(0, 0, 0,-150);
					
			m_pWndCombo->SetFont(GetFont());
			
			m_pWndCombo->MoveWindow(&rcEdit);
			BOOL bFound = FALSE;
			for (UINT pos = 0;pos<m_comboStrings.size();pos++)
			{
				m_pWndCombo->AddString(m_comboStrings[pos]);
				if(strTxt.CompareNoCase(m_comboStrings[pos]) == 0)
				{
					bFound = TRUE;
					m_pWndCombo->SetCurSel(pos);
				}
			}
	...
	case RC_BUTTON_CTRL:
		{
			...
			if (m_pWndButton->GetSafeHwnd() != NULL)
				m_pWndButton->DestroyWindow();

			if (!m_pWndButton->Create(m_strBtnCaption,WS_CHILD | WS_VISIBLE, CRect(0, 0, 1, 1), this, ID_BUTTON))
				return FALSE;
			...
			int btWidth = rcEdit.Width()/100*15; //15%
			rcEdit.left = rcEdit.right-btWidth;

			m_pWndButton->MoveWindow(&rcEdit);
...
BOOL CReportCtrl::EndEdit(BOOL bCommit)
{
	...
	CString str;
	BOOL bButtonCtrl = FALSE;

	const DWORD	EFMT = _GetHeaderTextEditFormat(m_ptEditting.y);
	switch(EFMT) 
	{
	case RC_EDIT_CTRL:
		{
	...
	case RC_COMBO_CTRL:
		{
			;
			m_pWndCombo->GetLBText(m_pWndCombo->GetCurSel(),str);
			break;
		}
	case RC_BUTTON_CTRL:
		{
	...
	if(m_pWndButton->GetSafeHwnd() != NULL)
		m_pWndButton->ShowWindow(SW_HIDE);
	m_ptEditting.x = -1;
	m_ptEditting.y = -1;
	return bChanged;
}
...
DWORD CReportCtrl::_GetHeaderTextEditFormat(int nColumn) const
{
	if (!HasColumnHeader())
		return RC_EDIT_CTRL;
	
	if (m_ColumnEditStyle.size() < nColumn)
		return RC_EDIT_CTRL;

	return m_ColumnEditStyle[nColumn];
}
...
///////////////////////////////////////////////////////////////////////////////
// SetCurSel
BOOL CReportCtrl::SetCurSel(int nItem)
{
...
BOOL CReportCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if((pHDN->hdr.code == HDN_BEGINTRACKW || pHDN->hdr.code == HDN_BEGINTRACKA)
		&& m_bLockColumn[pHDN->iItem])            // Prevent only first (col# 0) from resizing
	{
		*pResult = TRUE;                // disable tracking
		return TRUE;                    // Processed message
	}

	if( wParam==0 && (((NMHDR*)lParam)->code == NM_RCLICK) && m_bEnableHdrRClick)
	{
		POINT Point;
		GetCursorPos (&Point);
		...
		CListCtrl::GetHeaderCtrl()->SendMessage(HDM_HITTEST,0,(LPARAM)&HitTest);

		GetParent()->SendMessage(HDN_ITEMRCLICK,lParam,wParam);
	}

	return CListCtrl::OnNotify(wParam, lParam, pResult);
}

int CReportCtrl::FindSubItem(CString strSearch,int nCol)
{
	int nItems;
	CString strItem;
	nItems = GetItemCount();
	...
	{
		strItem=GetItemText(i,nCol);
		if(strItem.Compare(strSearch)==0)
			return i;
	}
	return -1;
}
void CReportCtrl::OnKillFocus(CWnd* pNewWnd)
{
	...
	/*if (pNewWnd != m_pWndCombo)
		EndEdit();*/
}
void CReportCtrl::OnKillFocusChild()
{
	EndEdit();
}

void CReportCtrl::OnBnClickedButton()
{
	int nPos = GetCurSel();
	GetParent()->SendMessage(WM_BUTTON_LCLICKED,nPos,NULL);
}
