...
namespace LJ.Controls.Win32
{
	...
	public class WindowsAPI
	{
		...
		public static extern IntPtr SHGetFileInfo(string drivePath, uint fileAttributes, 
			out SHFILEINFO fileInfo, uint countBytesFileInfo, uint flags);
		...
		[DllImport("Shell32.dll", CharSet=CharSet.Auto)]
		public static extern int SHGetSpecialFolderLocation(IntPtr hwndOwner, ShellFolders folder, out IntPtr idl);
		#endregion
		...
		public static extern int ImageList_DrawEx(IntPtr hImageList, int imageIndex,
			IntPtr hDCDest, int x, int y, int dx, int dy, uint backColor, uint foregColor, ImageListDrawFlags flags);
		#endregion
